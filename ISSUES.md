...
#### Fase 0: Preparación
1.  **Crear Rama Aislada:** Crear una nueva rama en Git dedicada exclusivamente a la actualización (ej. `feature/angular-20-upgrade`).
2.  **Limpieza del Entorno:** Eliminar `node_modules` y `package-lock.json` para asegurar un entorno de dependencias limpio. Ejecutar `npm install` para verificar que el proyecto base está estable.
3.  **Verificar Pruebas (si existen):** Ejecutar `ng test` para asegurar que el estado actual es conocido y funcional.

#### Fase 1: Actualización a Angular 18
1.  **Ejecutar Comandos de Actualización:**
    ```bash
    ng update @angular/core@18 @angular/cli@18
    ng update @angular/material@18
    ```
2.  **Análisis y Migración:**
...
3.  **Revisión Manual de Breaking Changes:**
...
4.  **Verificación:**
    ...
    - Ejecutar `ng test`.
5.  **Commit:** Una vez estable, hacer commit de la actualización a la v18: `git commit -m "feat(ng): Upgrade to Angular 18"`.
...
5.  **Commit:** `git commit -m "feat(ng): Upgrade to Angular 19"`.
...
5.  **Commit:** `git commit -m "feat(ng): Complete upgrade to Angular 20"`.
...
2.  **Limpieza de Código:** Eliminar cualquier solución temporal o código obsoleto introducido durante el proceso de actualización.
3.  **Merge:** Una vez que la rama `feature/angular-20-upgrade` sea 100% estable y probada, fusionarla con la rama de desarrollo principal.
